...
class TestProcessPipeline:
    ...
    def test_orbits_key_present_when_data_has_orbits(
        self, pipeline_mojito_data_with_orbits
    ):
        result = process_pipeline(
            pipeline_mojito_data_with_orbits,
            filter_kwargs={"highpass_cutoff": 0.01},
        )
        assert "orbits" in result

    def test_orbits_has_expected_keys(self, pipeline_mojito_data_with_orbits):
        result = process_pipeline(
            pipeline_mojito_data_with_orbits,
            filter_kwargs={"highpass_cutoff": 0.01},
        )
        ...
        assert set(result["orbits"].keys()) == expected

    def test_orbits_key_absent_when_no_orbit_data(self, pipeline_mojito_data):
        result = process_pipeline(
            pipeline_mojito_data,
            filter_kwargs={"highpass_cutoff": 0.01},
        )
        assert "orbits" not in result

    def test_orbits_times_unchanged(self, pipeline_mojito_data_with_orbits):
        """Orbit times must not be sliced — full array is passed through."""
        result = process_pipeline(
            pipeline_mojito_data_with_orbits,
            filter_kwargs={"highpass_cutoff": 0.01},
        )
        np.testing.assert_array_equal(
            result["orbits"]["times"],
            pipeline_mojito_data_with_orbits["orbit_times"],
        )

    def test_orbits_present_in_single_segment_path(
        self, pipeline_mojito_data_with_orbits
    ):
        """No-segmentation path must also include the orbits key."""
        result = process_pipeline(
            pipeline_mojito_data_with_orbits,
            filter_kwargs={"highpass_cutoff": 0.01},
        )
    ...
    def test_orbits_present_in_multi_segment_path(
        self, pipeline_mojito_data_with_orbits
    ):
        result = process_pipeline(
            pipeline_mojito_data_with_orbits,
    ...
    def test_segment_keys_unaffected_by_orbits(self, pipeline_mojito_data_with_orbits):
        ...
        assert isinstance(result["segment0"], SignalProcessor)

    def test_orbits_sc_position_shape(self, pipeline_mojito_data_with_orbits):
        result = process_pipeline(
            pipeline_mojito_data_with_orbits,
            filter_kwargs={"highpass_cutoff": 0.01},
        )
        n_orbit = len(pipeline_mojito_data_with_orbits["orbit_times"])
        assert result["orbits"]["sc_position_1"].shape == (n_orbit, 3)

    def test_orbits_absent_when_only_orbits_key_missing(
        self, pipeline_mojito_data_with_orbits
    ):
        data = dict(pipeline_mojito_data_with_orbits)
        del data["orbits"]
        result = process_pipeline(data, filter_kwargs={"highpass_cutoff": 0.01})
        assert "orbits" not in result
