...
class TimeSampling:
    ...
    def __repr__(self):
        return f"SimpleSampling(t0={self.t0}, dt={self.dt}, size={self.size}, fs={self.fs} Hz)"
...
class LogFrequencySampling:
    ...
    def __repr__(self):
        return f"SimpleLogFrequencySampling(fmin={self.fmin}, fmax={self.fmax}, size={self.size})"
...
class MojitoData:
    ...
    def __init__(self):
        ...
        self.noise_cov_aet = None  # Full (n_seg, n_freq, 3, 3) covariance (if available)
    ...
    def __repr__(self):
        ...
        s += f"  Samples: {self.n_samples:,} @ {self.fs} Hz ({self.duration/86400:.2f} days)\n"
        s += f"  Noise estimates: {len(self.noise_freqs)} freq bins from {self.noise_freqs[0]:.2e} to {self.noise_freqs[-1]:.2e} Hz\n"
...
class OrbitData:
    ...
    def __repr__(self):
        ...
        s += f"  LTT samples: {self.ltt_n_samples:,} @ {self.ltt_fs} Hz ({self.ltt_duration/86400:.2f} days)\n"
        s += f"  Spacecraft: {list(self.sc_positions.keys())}\n"
        s += f"  Orbit samples: {self.orbit_n_samples:,} @ {self.orbit_fs:.2e} Hz ({self.orbit_duration/86400:.2f} days)\n"
        s += f"  Metadata: {list(self.metadata.keys())}\n"
        return s
...
def load_mojito_l1(filepath):
    ...
    with h5py.File(filepath, 'r') as f:
        ...
        print(f"\nPre-computed Noise Estimates:")
        print(f"  Frequency range: {data.noise_freqs[0]:.2e} to {data.noise_freqs[-1]:.2e} Hz")
    ...
    with h5py.File(filepath, 'r') as f:
        ...
        if 'ltts' in f:
            ...
            if 'sampling' in ltt_group:
                ...
                print(f"  Duration: {data.ltt_duration:.1f} s ({data.ltt_duration/86400:.2f} days)")
        ...
        if 'orbits' in f:
            ...
            if 'sampling' in orbit_group:
                ...
                print(f"  Duration: {data.orbit_duration:.1f} s ({data.orbit_duration/86400:.2f} days)")
...
def truncate_mojito_data(data, duration_seconds=None, n_samples=None):
    ...
    if duration_seconds is None and n_samples is None:
        print("Warning: No truncation parameters specified. Returning copy of original data.")
        return data
    
    # Calculate n_samples from duration if needed
    if duration_seconds is not None:
        n_samples_truncated = int(duration_seconds / data.dt)
        print(f"Truncating to {duration_seconds:.1f} seconds = {n_samples_truncated:,} samples")
    else:
        n_samples_truncated = n_samples
        duration_truncated = n_samples_truncated * data.dt
        print(f"Truncating to {n_samples_truncated:,} samples = {duration_truncated:.1f} seconds ({duration_truncated/86400:.3f} days)")
    
    # Validate truncation length
    if n_samples_truncated > data.n_samples:
        raise ValueError(f"Requested {n_samples_truncated} samples but data only has {data.n_samples}")
    ...
    truncated.sampling = TimeSampling(
        t0=truncated.t0,
        dt=truncated.dt,
        size=n_samples_truncated
    )
    truncated.t_tdi = truncated.sampling.t()
    
    print(f"\nTruncated sampling:")
    print(f"  Original: {data.n_samples:,} samples, {data.duration:.1f} s ({data.duration/86400:.3f} days)")
    print(f"  Truncated: {truncated.n_samples:,} samples, {truncated.duration:.1f} s ({truncated.duration/86400:.3f} days)")
    print(f"  Kept: {100*n_samples_truncated/data.n_samples:.1f}% of original data")
    ...
    truncated.noise_freqs = data.noise_freqs.copy() if data.noise_freqs is not None else None
    truncated.noise_psds = data.noise_psds.copy()
    truncated.noise_cov_xyz = data.noise_cov_xyz.copy() if data.noise_cov_xyz is not None else None
    truncated.noise_cov_aet = data.noise_cov_aet.copy() if data.noise_cov_aet is not None else None
    
    print(f"\nNote: Noise estimates copied from original (computed on full dataset).")
    print(f"      For accurate noise characterization of truncated data, recompute PSDs.")
...
def truncate_orbit_data(data, duration_seconds=None, n_samples=None):
    ...
    - Sampling metadata (duration, n_samples, etc.) is updated
    - Orbit data (positions/velocities) are NOT truncated as they have different sampling
    ...
    if n_samples_truncated > data.ltt_n_samples:
        raise ValueError(f"Requested {n_samples_truncated} samples but LTT data only has {data.ltt_n_samples}")
    ...
    print(f"  Original: {data.ltt_n_samples:,} samples, {data.ltt_duration:.1f} s ({data.ltt_duration/86400:.3f} days)")
    print(f"  Truncated: {truncated.ltt_n_samples:,} samples, {truncated.ltt_duration:.1f} s ({truncated.ltt_duration/86400:.3f} days)")
    print(f"  Kept: {100*n_samples_truncated/data.ltt_n_samples:.1f}% of original data")
    ...
    for link, ltt_deriv_array in data.ltt_derivatives.items():
        ...
        print(f"  ltt_derivative_{link}: {ltt_deriv_array.shape} → {truncated.ltt_derivatives[link].shape}")
    ...
    print(f"\nNote: Spacecraft orbit data copied unchanged (different sampling: {data.orbit_fs:.2e} Hz)")
    print(f"\n✓ Truncation complete!")
